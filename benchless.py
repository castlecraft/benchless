...
@click.option(
    ...
    default=os.path.dirname(os.path.realpath(__file__))[1:].replace('/', '-'),
    show_default=True,
)
@click.option(
    '--bench-dir',
...
@click.option(
    ...
    default=os.path.join(os.path.dirname(os.path.realpath(__file__)), 'sites'),
    show_default=True,
)
@click.option(
    '--bench-cmd',
    help='Frappe base command',
    default=os.path.join(os.path.dirname(os.path.realpath(__file__)), 'benchless.py') + ' frappe',
    show_default=True,
)
@click.option(
    '--node',
...
def production(
    ...
    supervisor_conf=get_template(bench_dir, 'supervisor.conf.tmpl').render(render_opts)
...
def setup(
    ...
    common_config=get_template(bench_dir, 'common_site_config.json.tmpl').render(render_opts)
...
@main.command(help="Frappe commands", add_help_option=False, context_settings=dict(
...
def frappe(ctx):
    ...
    subprocess.check_call([
        sys.executable,
        os.path.join(bench_dir, 'apps', 'frappe', 'frappe', 'utils', 'bench_helper.py'),
        'frappe',
        *ctx.args
    ])
...
@click.option(
    '--repo',
    help='Git repository of app, required to add app'
)
@click.option(
    '--branch',
    default='develop',
